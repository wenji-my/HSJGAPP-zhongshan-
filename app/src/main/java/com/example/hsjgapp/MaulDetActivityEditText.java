...
public class MaulDetActivityEditText extends Activity implements
...
ExpandableListView.OnGroupClickListener, ExpandableListView.OnChildClickListener {
	...
	private MaulItemAdapterEditText exlist_adapter = null;
	...
	private Handler hander = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == MSG_START_FAIL) {
				DialogTool.AlertDialogShow(MaulDetActivityEditText.this, msg.obj.toString());
			}
			if (msg.what == MSG_START_SHOW) {
				builder2 = new ProgressDialog(MaulDetActivityEditText.this);
			...
			if (msg.what == MSG_START_CHAOSHI) {
				DialogTool.AlertDialogShow(MaulDetActivityEditText.this, "请求超时，请检查网络环境!");
			}
			...
			if (msg.what == MSG_START_ZIDINGYI) {
				AlertDialog.Builder builder1 = new AlertDialog.Builder(MaulDetActivityEditText.this);
				...
				builder1.setPositiveButton("确定", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if (xmname.equals("F1")) {
							...
							MaulDetActivityEditText.this.finish();
						}
					}
				});
				builder1.show();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		dtzhi = share.getBoolean("DTXZ", false);
		dpzhi = share.getBoolean("DPXZ", false);
		photozhi = share.getBoolean("PHOTOZ", true);
		timezhi = share.getBoolean("TIMETK", false);
		Bundle bundle = MaulDetActivityEditText.this.getIntent().getExtras();
		...
		if (xmpd) {
			...
			init();
			exlist_adapter = new MaulItemAdapterEditText(this, groups, number5, xmname, hander);
		...
		case R.id.btnSubmit:
			...
			new Thread(new Runnable() {
				...
				public void run() {
					 ...
					 * = getLpPhotodata(theinformation).get(0).getYpzp(); if
	...
	private void dialogs() {
		AlertDialog.Builder builder3 = new AlertDialog.Builder(MaulDetActivityEditText.this);
	...
	public List<String> getF1() {
		...
		for (int i = 0; i < PanDuan.wgjsxmdata.length; i++) {
			if (MaulItemAdapterEditText.map.get(PanDuan.wgjsxmname[i]) != null
					&& MaulItemAdapterEditText.map.get(PanDuan.wgjsxmname[i]).equals("2")) {
				if (MaulItemAdapterEditText.bhgxxmap.get(PanDuan.wgjsxmname[i]) != null) {
					String zpdata = PanDuan.wzZWsz(PanDuan.wgjsxmname[i]) + "$0$"
							+ MaulItemAdapterEditText.bhgxxmap.get(PanDuan.wgjsxmname[i]);
					bhesmdatas = bhesmdatas + UnXmlTool.getItems(zpdata);
				}
			}
			...
			if (is) {
				if (MaulItemAdapterEditText.map.get(PanDuan.wgjcxmpd(PanDuan.wgjsxmdata[i])) != null) {
					datalist2.add(MaulItemAdapterEditText.map.get(PanDuan.wgjcxmpd(PanDuan.wgjsxmdata[i])));
				} else {
					datalist2.add("1");
				}
				is = false;
	...
	public List<String> getDC() {
		...
		for (int i = 0; i < PanDuan.dtdpxmdata.length; i++) {
			if (MaulItemAdapterEditText.map.get(PanDuan.dtdpxmname[i]) != null
					&& MaulItemAdapterEditText.map.get(PanDuan.dtdpxmname[i]).equals("2")) {
				if (MaulItemAdapterEditText.bhgxxmap.get(PanDuan.dtdpxmname[i]) != null) {
					String zpdata = PanDuan.wzZWsz(PanDuan.dtdpxmname[i]) + "$0$"
							+ MaulItemAdapterEditText.bhgxxmap.get(PanDuan.dtdpxmname[i]);
					bhesmdatas = bhesmdatas + UnXmlTool.getItems(zpdata);
				}
			}
			...
			if (is) {
				if (MaulItemAdapterEditText.map.get(PanDuan.Dtdpxmpd(PanDuan.dtdpxmdata[i])) != null) {
					datalist2.add(MaulItemAdapterEditText.map.get(PanDuan.Dtdpxmpd(PanDuan.dtdpxmdata[i])));
				} else {
					datalist2.add("1");
				}
				is = false;
	...
	public List<String> getC1() {
		...
		for (int i = 0; i < PanDuan.dpjyxmdata.length; i++) {
			if (MaulItemAdapterEditText.map.get(PanDuan.dpjyxmname[i]) != null
					&& MaulItemAdapterEditText.map.get(PanDuan.dpjyxmname[i]).equals("2")) {
				if (MaulItemAdapterEditText.bhgxxmap.get(PanDuan.dpjyxmname[i]) != null) {
					String zpdata = PanDuan.wzZWsz(PanDuan.dpjyxmname[i]) + "$0$"
							+ MaulItemAdapterEditText.bhgxxmap.get(PanDuan.dpjyxmname[i]);
					bhesmdatas = bhesmdatas + UnXmlTool.getItems(zpdata);
				}
			}
			...
			if (is) {
				if (MaulItemAdapterEditText.map.get(PanDuan.Dpjyxmpd(PanDuan.dpjyxmdata[i])) != null) {
					datalist2.add(MaulItemAdapterEditText.map.get(PanDuan.Dpjyxmpd(PanDuan.dpjyxmdata[i])));
				} else {
					datalist2.add("1");
				}
				is = false;
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if (keyCode == KeyEvent.KEYCODE_BACK && event.getRepeatCount() == 0) {
			AlertDialog.Builder newbuilder = new AlertDialog.Builder(MaulDetActivityEditText.this);
